...
describe('isValidUrl', () => {
  ...
  it('recognizes browser internal schemes', () => {
    ...
    expect(isValidUrl('moz-extension://1234-uuid/index.html')).toBe('moz-extension://1234-uuid/index.html')
  ...
  it('recognizes Windows local file paths and converts to file:/// URL', () => {
    expect(isValidUrl('C:\\Users\\name\\file.txt')).toBe('file:///C:/Users/name/file.txt')
    expect(isValidUrl('D:/Documents/report.pdf')).toBe('file:///D:/Documents/report.pdf')
  })
  ...
  it('recognizes valid domain names and prepends https:// by default', () => {
    ...
    expect(isValidUrl('github.com/manikantjha/gear')).toBe('https://github.com/manikantjha/gear')
    expect(isValidUrl('https://news.ycombinator.com')).toBe('https://news.ycombinator.com/')
...
describe('getRedirectUrl', () => {
  it('returns valid URLs directly without running a search query', () => {
    ...
    expect(getRedirectUrl('https://example.com/test')).toBe('https://example.com/test')
    expect(getRedirectUrl('about:blank')).toBe('about:blank')
  })

  it('resolves built-in bangs to their respective search destinations', () => {
    expect(getRedirectUrl('!g react hooks')).toBe('https://www.google.com/search?q=react%20hooks')
    expect(getRedirectUrl('!wi typescript')).toBe('https://www.google.com/search?q=typescript+site:wikipedia.org')
    expect(getRedirectUrl('!yt lo-fi beats')).toBe('https://www.youtube.com/results?search_query=lo-fi%20beats')
  })

  it('falls back to the configured search provider when no bang is present', () => {
    expect(getRedirectUrl('learn rust', 'google')).toBe('https://www.google.com/search?q=learn%20rust')
    expect(getRedirectUrl('learn rust', 'duckduckgo')).toBe('https://duckduckgo.com/?q=learn%20rust')
    expect(getRedirectUrl('learn rust', 'bing')).toBe('https://www.bing.com/search?q=learn%20rust')
    expect(getRedirectUrl('learn rust', 'brave')).toBe('https://search.brave.com/search?q=learn%20rust')
  })

  it('falls back to default provider if unknown provider is supplied', () => {
    expect(getRedirectUrl('vite react', 'unknown' as any)).toBe('https://www.google.com/search?q=vite%20react')
  })
})
