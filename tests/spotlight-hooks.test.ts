...
describe('spotlight hooks and algorithms', () => {
  describe('getSpotlightResults', () => {
    const mockTabs = [
      { id: 1, windowId: 1, title: 'React Documentation', url: 'https://react.dev', active: false },
      { id: 2, windowId: 1, title: 'GitHub', url: 'https://github.com', active: false },
    ]
    const mockBookmarks = [
      { id: 'b1', title: 'React Icons', url: 'https://react-icons.github.io' },
    ]
    const mockHistory = [
      { id: 'h1', title: 'React Tutorial', url: 'https://react.dev/learn', lastVisitTime: 100 },
    ]
    const mockSuggestions = [
      { id: 's1', query: 'react 19 features', title: 'react 19 features', url: 'https://google.com/search?q=react' },
    ]
    ...
    it('deduplicates identical URLs across sources', () => {
      const duplicateHistory = [
        { id: 'h_dup', title: 'React Docs Duplicate', url: 'https://react.dev' },
      ]
      ...
      const reactDevMatches = results.filter((r) => r.url === 'https://react.dev')
  ...
  describe('useOpenTabs', () => {
    it('fetches open tabs when spotlight is open', async () => {
      const mockTabs = [{ id: 1, title: 'Tab 1', url: 'https://tab1.com', windowId: 1, active: true }]
  ...
  describe('useBookmarks', () => {
    it('fetches bookmarks when enabled and open', () => {
      const mockBookmarks = [{ id: 'b1', title: 'BM 1', url: 'https://bm1.com' }]
      ;(chrome.runtime.sendMessage as any).mockImplementation((msg: any, cb: any) => {
      ...
      const { result } = renderHook(() =>
        useBookmarks({ isOpen: true, rawQuery: '', cleanQuery: '', enabled: true })
      )
  ...
  describe('useHistory', () => {
    it('fetches history when enabled and open', () => {
      const mockHistory = [{ id: 'h1', title: 'History 1', url: 'https://h1.com' }]
      ;(chrome.runtime.sendMessage as any).mockImplementation((msg: any, cb: any) => {
