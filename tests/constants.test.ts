...
describe('constants and isRestrictedUrl', () => {
  ...
  it('restricts Chrome and Mozilla extension stores', () => {
    for (const domain of RESTRICTED_URL_DOMAINS) {
      expect(isRestrictedUrl(`https://${domain}/detail/some-extension`)).toBe(true)
    }
  })
  ...
  it('defines valid command names and message types', () => {
    ...
    expect(SEARCH_RESULT_PRIORITY.DIRECT_URL).toBeLessThan(SEARCH_RESULT_PRIORITY.OPEN_TAB)
    expect(SEARCH_RESULT_PRIORITY.OPEN_TAB).toBeLessThan(SEARCH_RESULT_PRIORITY.BOOKMARK)
  })
})
