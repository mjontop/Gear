...
describe('Spotlight UI and Views', () => {
  ...
  describe('Spotlight root component', () => {
    ...
    it('handles keyboard navigation and tab autocomplete in Spotlight', async () => {
      ...
      ;(globalThis as any).chrome.runtime.sendMessage = vi.fn((msg: any, cb: any) => {
        if (msg.type === MESSAGE_TYPES.GET_OPEN_TABS && cb) {
          cb({ tabs: [{ id: 10, title: 'Open Tab Item', url: 'https://example.com/tab', windowId: 1, active: true }] })
        }
      })
      ...
      await act(async () => {
        messageListener({ type: MESSAGE_TYPES.OPEN_SPOTLIGHT_WITH_URL, url: 'https://newsite.com' })
      })
