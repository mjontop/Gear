...
describe('bangs-storage', () => {
  ...
  describe('validateBang', () => {
    const existingBangs = { '!g': 'https://google.com?q=%s', '!yt': 'https://youtube.com?q=%s' }
    ...
    it('flags invalid characters in prefix', () => {
      const res = validateBang('!my bang$', 'https://test.com?q=%s', existingBangs)
      expect(res.isValid).toBe(false)
    ...
    it('allows duplicate prefix when editing the same bang', () => {
      const res = validateBang('!g', 'https://google.com/search?q=%s', existingBangs, '!g')
    ...
    it('flags URL missing %s placeholder', () => {
      const res = validateBang('!test', 'https://test.com/search', existingBangs)
      expect(res.isValid).toBe(false)
      expect(res.urlError).toContain("must include '%s'")
    })

    it('accepts valid bang definitions', () => {
      const res = validateBang('!npm', 'https://www.npmjs.com/search?q=%s', existingBangs)
      expect(res.isValid).toBe(true)
  ...
  describe('storage methods', () => {
    ...
    it('catches and handles storage read errors gracefully', async () => {
      ...
      ;(globalThis as any).chrome.storage.sync.get = vi.fn().mockRejectedValue(new Error('disk error'))
