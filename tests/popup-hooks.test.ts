...
describe('popup custom hooks', () => {
  describe('useBangsManager', () => {
    it('initializes with default bangs and allows adding custom bang', async () => {
      ...
      expect(result.current.allBangs['!gh']).toBe('https://github.com/search?q=%s')
    })
  ...
  describe('useBookmarksManager', () => {
    beforeEach(() => {
      ;(globalThis as any).chrome.bookmarks.getRecent = vi.fn().mockResolvedValue([
        { id: 'bm1', title: 'Vitest', url: 'https://vitest.dev', dateAdded: 100 },
      ])
    })
    ...
    it('adds bookmark through hook', async () => {
      ...
      expect(onStatus).toHaveBeenCalledWith("Added bookmark 'MDN Docs'", 'success')
    })
  })
})
